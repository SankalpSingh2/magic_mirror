...
class CustomGo2Env:
    def __init__(self, num_envs, env_cfg, obs_cfg, reward_cfg, command_cfg, show_viewer=False, custom_scene=None):
        ...
        # Use custom scene if provided, otherwise create a new one
        if custom_scene:
        ...
        # build only if we created our own scene
        if not custom_scene:
            self.scene.build(n_envs=num_envs)
        else:
            # If we're using a custom scene, it might have already been built
            # or needs to be built after adding the robot
    ...
    def reset_idx(self, envs_idx):
        ...
        # reset base
        self.base_pos[envs_idx] = self.base_init_pos.to(torch.float32)
